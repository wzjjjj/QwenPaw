...
### 1.1.5 补充：渠道侧的“真实世界”细节（多媒体输入、重连、命令解析）

当你把渠道接入到真实 IM 平台后，最常见的问题往往不在“跑不跑得通”，而在这些边缘细节：消息类型不稳定、连接会断、平台会重试、命令格式会被平台改写。1.1.5 里一些关键改动可以作为你写新渠道时的对照清单：

- **QQ 语音与 ASR**：通过文件扩展名识别语音附件；平台给出 ASR 文本时直接当作 TextContent 使用（避免下载/转码成本）；无 ASR 时优先使用预转换的 wav 链接  
  - [qq/channel.py](file:///d:/编程学习记录/QwenPaw/src/qwenpaw/app/channels/qq/channel.py#L1237-L1321)
- **QQ WebSocket 自动重连**：可恢复错误触发重连；对“快速断连”做节流与 token 刷新策略（避免进入高频重连死循环）  
  - [qq/channel.py](file:///d:/编程学习记录/QwenPaw/src/qwenpaw/app/channels/qq/channel.py#L1600-L1741)
- **频道内审批命令可用性**：`/approve`、`/deny` 在 Channels 内需要被正确识别为命令而非普通文本（典型坑：命令解析只在控制台链路生效）
- **@提及导致的命令变形**（企业微信等）：群聊里“@机器人 + 斜杠命令”常被平台改写为带 mention 前缀的文本，命令解析需要对这种输入做归一化
- **表情回应等事件**（飞书等）：有些平台会推送“非消息”的事件（reaction、read、typing），最稳妥的策略是注册空处理器并显式忽略，避免把系统拖进异常分支
